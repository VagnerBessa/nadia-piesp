...
async function test() {
  ...
  const response = await fetch(OPENROUTER_URL, {
    ...
    body: JSON.stringify({
      ...
      messages: [{ role: 'system', content: 'Use the dataset empresas-sp-mar26-20260522 e a coluna cnpj' }, { role: 'user', content: 'Qual o número de empresas?' }],
      tools: tools,
      tool_choice: 'auto'
    })
  });
  ...
  console.log("GPT Tool Call:", JSON.stringify(json.choices[0].message.tool_calls, null, 2));
}
