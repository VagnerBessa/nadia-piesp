...
export const useLiveConnection = ({ systemInstruction, tools, onToolCall }: UseLiveConnectionOptions = {}) => {
  ...
  const startConversation = useCallback(async () => {
    ...
    try {
      ...
      // Adicionando a Diretriz de UX/Voice com Trava Psicológica Estrita
      finalSystemInstruction += `\n\n[COMPORTAMENTO ACÚSTICO E BUSCA DE DADOS]\nREGRA DE APRESENTAÇÃO: Se o usuário iniciar a conversa te chamando pelo nome (Nadia), NUNCA se apresente dizendo quem você é. Vá direto ao assunto.\nIMPORTANTE SOBRE FERRAMENTAS: Se você precisar consultar a base de dados do PIESP, você DEVE dizer em voz alta uma frase preenchedora MUITO CURTA (ex: "Um instante", "Buscando...", "Deixe-me ver na base..."). \nPROIBIÇÃO ABSOLUTA: Durante essa primeira fala antes da ferramenta, é TERMINANTEMENTE PROIBIDO começar a responder a pergunta do usuário ou despejar conhecimento genérico. O primeiro áudio serve EXCLUSIVAMENTE como uma confirmação de espera. Morda a língua e não fale sobre o tema. Somente APÓS a ferramenta devolver os dados, faça uma nova fala entregando a resposta profunda e analítica. \nREGRAS GERAIS DE VOZ: O canal de áudio exige respostas diretas (máximo de 2 parágrafos). Vá direto ao ponto matemático.`;
