...
export const useLiveConnection = ({ systemInstruction, tools, onToolCall }: UseLiveConnectionOptions = {}) => {
  ...
  const startConversation = useCallback(async () => {
    ...
    try {
      ...
      // Adicionando a Diretriz de UX/Voice para mitigação de "Resposta Abrupta" e TAMANHO (O Feedback Pré-Tool agora é Visual pelo React)
      finalSystemInstruction += `\n\n[COMPORTAMENTO ACÚSTICO E BUSCA DE DADOS]\nREGRA DE APRESENTAÇÃO: Se o usuário iniciar a conversa já te chamando pelo nome (Nadia), NUNCA se apresente de volta dizendo quem você é. Vá direto ao que ele perguntou.\nIMPORTANTE: É ESTRITAMENTE PROIBIDO inventar ou adivinhar informações do PIESP; se você precisar buscar dados da empresa, estado ou valores, dispare a ferramenta SILENCIOSAMENTE E IMEDIATAMENTE antes de falar qualquer coisa. Nós cuidaremos de dar o aviso visual de "Buscando informações" na tela para o usuário. Assim que a ferramenta lhe devolver os números, você pode iniciar sua fala com uma frase orgânica (Ex: "Aqui está...") e responder a análise.\nREGRAS GERAIS DE VOZ: O canal de áudio não tolera discursos longos. As suas respostas finais analíticas DEVEM SER BASTANTE BREVES E DIRETAS (máximo de 2 parágrafos). Vá direto ao ponto matemático e insight principal. Se houver tom de voz alterado, mantenha o tom profissional idêntico antes e depois do uso da ferramenta.`;
